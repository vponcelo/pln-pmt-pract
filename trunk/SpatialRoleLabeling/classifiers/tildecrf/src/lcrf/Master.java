...
public class Master {
    ...
    public static Options getOptions() {
        ...
        options.addOption(OptionBuilder.hasArg().withLongOpt("mode").isRequired().withDescription(
                "operation mode, possible values are: " + Master.MODETAGGING + ", "
                        + Master.MODECLASSIFICATION + ", " + Master.MODECLASSIFICATIONOAA + ", "
                        + Master.MODECCLASSIFICATIONRR).isRequired().create('m'));
     ...
     * @return
     */
    private static SimpleExampleContainer processTaggingExamples(Logger logger, String[] entries) {
        SimpleExampleContainer allExamplesTagging = null;

        for (String entry : entries) {
            logger.info("Read tagging examples from file " + entry);
            SimpleExampleContainer tmp = XMLInput.readInOutExamplesSAX(entry);
    ...
    public static void main(String[] args) {
        ...
        if ((new File(Master.PROPERTYFILE)).exists()) {
            PropertyConfigurator.configure(Master.PROPERTYFILE);
        } else {
        ...
        if (!(mode.equals(Master.MODETAGGING) || mode.equals(Master.MODECLASSIFICATION)
                || mode.equals(Master.MODECLASSIFICATIONOAA) || mode.equals(Master.MODECCLASSIFICATIONRR))) {
        ...
        // read the data
        SimpleExampleContainer trainExamplesTagging = null;
        SimpleExampleContainer testExamplesTagging = null;
        ...
        if (line.hasOption("data")) {
            ...
            if (mode.equals(Master.MODETAGGING)) {
                SimpleExampleContainer tmp = Master.processTaggingExamples(preparationLogger, entries);
                trainExamplesTagging = tmp.getSubfoldInverse(folds, fold, seed);
                testExamplesTagging = tmp.getSubfold(folds, fold, seed);
            } else {
                ClassificationExampleContainer tmp = Master.processClassificationExamples(preparationLogger,
                        entries);
            ...
            // read the data from the files
            if (mode.equals(Master.MODETAGGING)) {
                trainExamplesTagging = Master.processTaggingExamples(preparationLogger, line
                        .getOptionValues("train"));
                testExamplesTagging = Master.processTaggingExamples(preparationLogger, line
                        .getOptionValues("test"));
            } else {
                trainExamplesClassification = Master.processClassificationExamples(preparationLogger, line
                        .getOptionValues("train"));
                testExamplesClassification = Master.processClassificationExamples(preparationLogger, line
                        .getOptionValues("test"));
                trainExamplesClassification.sortClassAtoms();
                testExamplesClassification.sortClassAtoms();
            }
        ...
        if (line.hasOption("validation")) {
            ...
            if (mode.equals(Master.MODETAGGING)) {
                ...
                validationExamplesClassification = Master.processClassificationExamples(preparationLogger, line
        ...
        if (schemaFile != null) {
            ...
            // in schema file!
            if (mode.equals(Master.MODECLASSIFICATION)) {
            ...
            if (mode.equals(Master.MODECLASSIFICATIONOAA) || mode.equals(Master.MODECCLASSIFICATIONRR)) {
        ...
        RegressionModelTrainer<List<Atom>> trainer = new TrainerWrapper<List<Atom>>(
                new LogicalRegressionTreeTrainer(treedepth, minleafsize, schemata, wm.getFieldTypes()), seed,
        ...
        // mode
        if (mode.equals(Master.MODETAGGING)) {
            new ModeTagging(trainExamplesTagging, testExamplesTagging, seed, trainer, wm, trainer2,
                    crftrainsteps);
        } else if (mode.equals(Master.MODECLASSIFICATION)) {
        ...
        } else if (mode.equals(Master.MODECLASSIFICATIONOAA)) {
        ...
        } else if (mode.equals(Master.MODECCLASSIFICATIONRR)) {
    ...
    private static void showHelp(String errText) {
        ...
        f.printHelp("java -jar tildecrf.jar", "Options", getOptions(), "\n\n(c) Bernd Gutmann and Kristian Kersting, 2006", true);
